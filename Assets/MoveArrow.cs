...
public class MoveArrow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!Input.GetKey(KeyCode.LeftArrow) && Input.GetKey(KeyCode.RightArrow) && transform.position.x < limit_vertical)
        {
            step_size_horizontal += acceleration;
            transform.position += new Vector3(step_size_horizontal * Time.deltaTime, 0, 0);
        }
        else if (Input.GetKey(KeyCode.LeftArrow) && !Input.GetKey(KeyCode.RightArrow) && transform.position.x > -limit_vertical)
        {
            step_size_horizontal += acceleration;
            transform.position += new Vector3(-step_size_horizontal * Time.deltaTime, 0, 0);
        }
        if (!Input.GetKey(KeyCode.DownArrow) && Input.GetKey(KeyCode.UpArrow) && transform.position.y < limit_horizontal)
        {
            step_size_vertical += acceleration;
            transform.position += new Vector3(0, step_size_vertical * Time.deltaTime, 0);
        }
        else if (Input.GetKey(KeyCode.DownArrow) && !Input.GetKey(KeyCode.UpArrow) && transform.position.y > -limit_horizontal)
        {
            step_size_vertical += acceleration;
            transform.position += new Vector3(0, -step_size_vertical * Time.deltaTime, 0);
        }
